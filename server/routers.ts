...
export const appRouter = router({
  ...
  news: router({
    ...
    create: protectedProcedure.input(z.object({ title: z.string().min(3), excerpt: z.string().min(3), content: z.string().min(3), category: z.string().min(2), image: z.object({ name: z.string(), type: z.string(), data: z.string() }).optional(), document: z.object({ name: z.string(), type: z.string(), data: z.string() }).optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) return { success: true, demo: true }; let imageKey: string | undefined; let imageUrl: string | undefined; let documentKey: string | undefined; let documentUrl: string | undefined; if (input.image) { const uploaded = await storagePut(`news/${ctx.user.id}/images/${input.image.name}`, Buffer.from(input.image.data, "base64"), input.image.type); imageKey = uploaded.key; imageUrl = uploaded.url; } if (input.document) { const uploaded = await storagePut(`news/${ctx.user.id}/documents/${input.document.name}`, Buffer.from(input.document.data, "base64"), input.document.type); documentKey = uploaded.key; documentUrl = uploaded.url; } await db.insert(news).values({ title: input.title, excerpt: input.excerpt, content: input.content, category: input.category, authorName: ctx.user.name ?? "Pengurus", imageKey, imageUrl, documentKey, documentUrl, documentName: input.document?.name }); return { success: true }; }),
  }),
});
